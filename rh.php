...
$ctido="SELECT * from rh where estado='1'";
$ctd=mysqli_query($conexion,$ctido);

$fgt="SELECT * from rh where estado='0'";
...
<div class="container">
	...
	<h1 class="page-header text-center">RH</h1>
	<div class="row">
		<div class="col-sm-12">
			...
			<table class="table table-bordered table-striped" style="margin-top:20px;">
				...
				<tbody>
					<?php
    					...
    					$db = $database->open();
						try{	
						    $sql = 'SELECT * FROM rh WHERE estado="1"';
						    foreach ($db->query($sql) as $row) {
						    	?>
						    	<tr>
						    		<td><?php echo $row['idrh']; ?></td>
						    		<td><?php echo $row['nombre']; ?></td>
						    		
						    		<td>
						    			<a href="#edit_<?php echo $row['idrh']; ?>" class="btn btn-success btn-sm" data-toggle="modal"><span class="fa fa-edit"></span> Editar</a>
						    			<a href="#delete_<?php echo $row['idrh']; ?>" class="btn btn-danger btn-sm" data-toggle="modal"><span class="fa fa-trash"></span> Eliminar</a>
						    		</td>
						    		<?php include('edit_delete_modalrh.php'); ?>
						    	</tr>
						    	<?php 
						    }
						}
